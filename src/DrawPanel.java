    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 3; c++) {
                ...
                Rectangle hit = new Rectangle(x * r * 2+9, y * c * 10, 45, 64);
                currentCards[r][c].setHitbox(hit);
                if (currentCards[r][c].getHitOn()) {
                    g.drawRect(x * r * 2 + 9, y * c * 10, 45, 64);
                }
            }
        }
        g.drawString("Number of cards left: " + unusedDeck.size(), x + 200, y + 200);
    }
